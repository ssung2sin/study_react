      ...
      <div style={{fontSize:'20px'}}>
        ...
        <label>
          <input type='radio' name='Comp' defaultValue='2'
                 onClick={selectComp}/> TwoApp
        </label>&nbsp;
        <label>
          <input type='radio' name='Comp' defaultValue='3'
                 onClick={selectComp}/> ThreeApp
        </label>&nbsp;
        <label>
          <input type='radio' name='Comp' defaultValue='4'
                 onClick={selectComp}/> FourApp
        </label>&nbsp;
        <label>
          <input type='radio' name='Comp' defaultValue='5'
                 onClick={selectComp}/> FiveApp
        </label>&nbsp;
        <label>
          <input type='radio' name='Comp' defaultValue='6'
                 onClick={selectComp}/> SixApp
        </label>&nbsp;
        <label>
          <input type='radio' name='Comp' defaultValue='7'
                 onClick={selectComp}/> SevenApp
        </label>&nbsp;

        <hr/>
        {idx===1?<OneApp/>:idx===2?<TwoApp/>:idx===3?<ThreeApp/>:idx===4?<FourApp/>:idx===5?<FiveApp/>:idx===6?<SixApp/>
            :<SevenApp/>}
      </div>
    </div>
  );
